#!/Users/zaz/Documents/work/exam-shell/build/venv/bin/python
# ===============================================================================
#
...
def get_lan_ip():
    ...
    def ip_from_gethostname():
        ip = socket.gethostbyname(socket.gethostname())
        return ip

    try:
        ip = ip_from_gethostname()
    except:
        ip = None

    if ip is None or ip.startswith("127."):
        try:
            ip = ip_from_interfaces()
        except:
            ip = None

    try:
        validated_ip = IPAddress(ip)
    except:
        raise Exception("Could not create a valid IP address from '{}'".format(ip))
